...
static void
usteer_check_band_steering_enabled(struct usteer_local_node *ln)
{
	// set to false in case list isn't specified.
	bool band_steer_disabled = config.band_steer_ssid_list;
	struct blob_attr *cur;
	int rem = 0;

	blobmsg_for_each_attr(cur, config.band_steer_ssid_list, rem) {
		if (strcmp(blobmsg_get_string(cur), ln->node.ssid) != 0)
			continue;

		band_steer_disabled = false;
		break;
	}

	ln->node.band_steer_disabled = band_steer_disabled;
}
...
usteer_register_node(struct ubus_context *ctx, const char *name, uint32_t id)
{
	...
	ln->node.band_steer_disabled = true;
	usteer_check_band_steering_enabled(ln);
}
...
void config_set_band_steer_ssid_list(struct blob_attr *data)
{
	...
	free(config.band_steer_ssid_list);

	if (data && blobmsg_len(data))
		config.band_steer_ssid_list = blob_memdup(data);
	else
		config.band_steer_ssid_list = NULL;
...
{
	if (config.band_steer_ssid_list)
		blobmsg_add_blob(buf, config.band_steer_ssid_list);
}
